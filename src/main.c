...
// Helper function to detect the programming language from the filename
// extension.
...
// Helper function to draw the window header with properly rounded bottom
// corners.
void draw_header(cairo_t *cr, double x, double y, double width, double height,
                 double radius, gboolean use_gradient) {
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp(argv[i], "-lang") == 0) {
      ...
      if (i + 1 < argc) {
        ...
        fprintf(stderr, "-lang option requires a language argument.\n");
        return 1;
      }
  ...
  if (input_filename == NULL || output_filename == NULL) {
    ...
    fprintf(
        stderr,
        "  -lang <language>  Specify language (see -list-lang for options).\n");
    fprintf(stderr, "  -list-lang        List supported languages.\n");
    fprintf(stderr, "  -no-gradient      Disable the gradient effect on the "
                    "window header.\n");
  ...
  if (lang == LANG_UNKNOWN && !no_color) {
    fprintf(stderr,
            "Error: Unsupported file type or language not specified.\n");
    fprintf(stderr, "This program currently only supports .c, .py, and .go "
                    "files for syntax highlighting.\n");
    fprintf(stderr, "Please use a supported file, specify the language with "
                    "-lang c|python|go, or use -no-color.\n");
    return 1;
  }
  ...
  cairo_surface_t *temp_surface =
      cairo_image_surface_create(CAIRO_FORMAT_A8, 0, 0);
  ...
  char *highlighted_text =
      highlight_syntax(code_content, lang, show_line_numbers, no_color);
  if (!highlighted_text) { // Check for memory allocation failure from
                           // highlight_syntax
    fprintf(stderr, "Error: Failed to highlight syntax due to memory "
                    "allocation failure.\n");
    g_free(code_content);
  ...
  cairo_pattern_t *pat =
      cairo_pattern_create_linear(0, 0, img_width, img_height);
  ...
  draw_rounded_rectangle(cr, PADDING / 2 + SHADOW_OFFSET,
                         PADDING / 2 + SHADOW_OFFSET, img_width - PADDING,
                         img_height - PADDING, BORDER_RADIUS);
  cairo_fill(cr);

  cairo_set_source_rgb(cr, 0.141, 0.157, 0.231);
  draw_rounded_rectangle(cr, PADDING / 2, PADDING / 2, img_width - PADDING,
                         img_height - PADDING, BORDER_RADIUS);
  cairo_fill(cr);

  // Draw the header using the new, improved function.
  draw_header(cr, PADDING / 2, PADDING / 2, img_width - PADDING, HEADER_HEIGHT,
              BORDER_RADIUS, use_gradient_header);

  cairo_set_source_rgb(cr, 0.9686, 0.4627, 0.5569); // Red
  cairo_arc(cr, PADDING / 2 + 20, PADDING / 2 + HEADER_HEIGHT / 2, 7, 0,
            2 * M_PI);
  cairo_fill(cr);

  cairo_set_source_rgb(cr, 0.8784, 0.6863, 0.4078); // Yellow
  cairo_arc(cr, PADDING / 2 + 45, PADDING / 2 + HEADER_HEIGHT / 2, 7, 0,
            2 * M_PI);
  cairo_fill(cr);

  cairo_set_source_rgb(cr, 0.6196, 0.8078, 0.4157); // Green
  cairo_arc(cr, PADDING / 2 + 70, PADDING / 2 + HEADER_HEIGHT / 2, 7, 0,
            2 * M_PI);
  cairo_fill(cr);
