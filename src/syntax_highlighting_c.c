...
static gboolean append_and_highlight(GString *highlighted_code,
  ...
  if (color) {
    g_string_append_printf(highlighted_code, "<span foreground='%s'>%s</span>",
                           color, escaped_token);
  } else {
...
void init_syntax_tables_c() {
  ...
  const char *keywords[] = {
      ...
      "double", "double_t", "else", "enum", "extern", "float", "float8",
      "float_t", "for", "goto", "if", "int", "int64_t", "uint_t", "uint64_t",
      "uint8_t", "bool", "long", "register", "return", "short", "signed",
  ...
  const char *preprocessor_directives[] = {
      ...
      "#elif",    "#endif",  "#error", "#pragma", "#line",  NULL};
  ...
  const char *standard_functions[] = {
      ...
      "vprintf", "vfprintf", "vsprintf", "vsnprintf", "fgetc", "fputc", "fgets",
      "fputs", "getc", "getchar", "gets", "putc", "putchar", "puts", "ungetc",
      "fopen", "freopen", "fclose", "fflush", "setbuf", "setvbuf", "fread",
      ...
      "getenv", "system", "bsearch", "qsort", "abs", "labs", "llabs", "div",
      "ldiv", "lldiv",

      // <string.h>
      "strcpy", "strncpy", "strcat", "strncat", "strlen", "strcmp", "strncmp",
      "strchr", "strrchr", "strstr", "strtok", "strspn", "strcspn", "strpbrk",
      "strerror", "memset", "memcpy", "memmove", "memcmp", "memchr",

      // <math.h>
      "sin", "cos", "tan", "asin", "acos", "atan", "atan2", "sinh", "cosh",
      "tanh", "exp", "log", "log10", "pow", "sqrt", "ceil", "floor", "fmod",
      "modf", "frexp", "ldexp", "fabs", "hypot", "fmax", "fmin", "fdim",
      "trunc", "round", "nearbyint", "rint", "copysign", "nan", "nextafter",
      "nexttoward", "fma", "log1p", "logb", "ilogb", "expm1", "cbrt", "erf",
      "erfc", "lgamma", "tgamma",

      // <time.h>
      "clock", "time", "difftime", "mktime", "gmtime", "localtime", "asctime",
      "ctime", "strftime",
      ...
      "wcschr", "wcsrchr", "wcswcs", "wcstok", "wcsftime", "wctomb", "mbstowcs",
      "wcstombs",
      ...
      // <fontconfig.h>
      "FcInit", "FcFini", "FcInitLoadConfig", "FcInitLoadConfigAndFonts",
      "FcMatchFont", "FcFontMatch", "FcFontList", "FcPatternCreate",
      "FcPatternAddString", "FcPatternAddBool", "FcPatternGetConfig",
...
static gboolean highlight_tokens_on_line(GString *highlighted_line_gstring,
  ...
  while (*ptr != '\0') {
    ...
    if (*in_multiline_comment) {
      ...
      if (!append_and_highlight(highlighted_line_gstring, start_of_plain_text,
                                current_token_start, "#545c7e", token_len))
        return FALSE;
      ptr += token_len;
    ...
    if (*ptr == '#') {
      ...
      if (strcmp(directive_word, "#include") == 0) {
        ...
        if (*ptr == '<' || *ptr == '"') {
          ...
          if (!append_and_highlight(highlighted_line_gstring, directive_end,
                                    header_start, "#9ece6a",
    ...
    if (token_len > 0) {
      if (!append_and_highlight(highlighted_line_gstring, start_of_plain_text,
                                current_token_start, token_color, token_len))
        return FALSE;
      ptr += token_len;
      start_of_plain_text = ptr;
    } else {
  ...
  for (char **line_ptr = code_lines; *line_ptr != NULL; line_ptr++) {
    ...
    if (show_line_numbers) {
      ...
      g_string_append_printf(current_line_gstring,
                             ...
                             line_number_width, current_line_num);
    }
