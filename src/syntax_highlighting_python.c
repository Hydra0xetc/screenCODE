...
void init_syntax_tables_python() {
  ...
  const char *standard_functions[] = {
      "print",      "input",     "len",       "range",       "sum",
      "max",        "min",       "abs",       "round",       "open",
      "close",      "read",      "write",     "append",      "strip",
      "split",      "join",      "int",       "float",       "str",
      "list",       "tuple",     "dict",      "set",         "bool",
      "type",       "id",        "dir",       "help",        "isinstance",
      "issubclass", "super",     "hasattr",   "getattr",     "setattr",
      "delattr",    "callable",  "frozenset", "complex",     "divmod",
      "enumerate",  "filter",    "map",       "next",        "iter",
  ...
  while (*ptr != '\0') {
    ...
    if (token_len == 0) { // If not handled by f-string or multiline string logic
      ...
      // 3. Numbers
        ...
        if (*num_scan_ptr == '0' &&
            (*(num_scan_ptr + 1) == 'x' || *(num_scan_ptr + 1) == 'X')) { // Hex
          num_scan_ptr += 2;
