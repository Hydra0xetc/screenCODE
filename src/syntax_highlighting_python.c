 ...
 */
static gboolean append_and_highlight(GString *highlighted_code, const char *start_of_plain_text,
                                     const char *current_token_start, const char *color, size_t len) {
...
void init_syntax_tables_python() {
    ...
    const char *keywords[] = {"import", "False",    "None",  "True",     "and",    "as",    "assert", "async",
                              "await",  "break",    "class", "continue", "def",    "del",   "elif",   "else",
                              "except", "finally",  "for",   "from",     "global", "if",    "in",     "is",
    ...
    const char *standard_functions[] = {
        "print",      "input",     "len",        "range",      "sum",         "max",
        "min",        "abs",       "round",      "open",       "close",       "read",
        "write",      "append",    "strip",      "split",      "join",        "int",
        "float",      "str",       "list",       "tuple",      "dict",        "set",
        "bool",       "type",      "id",         "dir",        "help",        "isinstance",
        "issubclass", "super",     "hasattr",    "getattr",    "setattr",     "delattr",
        "callable",   "frozenset", "complex",    "divmod",     "enumerate",   "filter",
        "map",        "next",      "iter",       "pow",        "reversed",    "slice",
        "sorted",     "zip",       "__import__", "any",        "all",         "chr",
        "ord",        "hex",       "oct",        "bin",        "classmethod", "staticmethod",
        "property",   "bytearray", "bytes",      "memoryview", "system",      "ascii",
        "breakpoint", "compile",   "eval",       "exec",       "format",      "globals",
        "locals",     "repr",      "vars",       "aiter",      "anext",       "__build_class__",
        "__debug__",  "__doc__",   "__loader__", "__name__",   "__package__", "__spec__",
        "copyright",  "credits",   "exit",       "license",    "quit",        "acos",
        "acosh",      "asin",      "asinh",      "atan",       "atan2",       "atanh",
        "ceil",       "comb",      "copysign",   "cos",        "cosh",        "degrees",
        "dist",       "erf",       "erfc",       "exp",        "expm1",       "fabs",
        "factorial",  "floor",     "fmod",       "frexp",      "fsum",        "gamma",
        "gcd",        "hypot",     "isclose",    "isfinite",   "isinf",       "isnan",
        "isqrt",      "ldexp",     "lgamma",     "log",        "log10",       "log1p",
        "log2",       "modf",      "perm",       "pow",        "prod",        "radians",
...
static gboolean highlight_tokens_on_line_python(GString *highlighted_line_gstring, const char *line_content,
    ...
    while (*ptr != '\0') {
        ...
        if (*in_multiline_string != 0) {
            ...
            while (*scan_ptr != '\0' &&
                   !(*scan_ptr == quote_char && *(scan_ptr + 1) == quote_char && *(scan_ptr + 2) == quote_char)) {
                scan_ptr++;
            }
        ...
        if (token_len == 0) { // If not handled by f-string or multiline string logic
                ...
                if (*num_scan_ptr == '0' && (*(num_scan_ptr + 1) == 'x' || *(num_scan_ptr + 1) == 'X')) { // Hex
                    ...
                    while (g_ascii_isxdigit(*num_scan_ptr))
                        num_scan_ptr++;
                           ...
                           (*(num_scan_ptr + 1) == 'o' || *(num_scan_ptr + 1) == 'O')) { // Octal
                    num_scan_ptr += 2;
                    while (*num_scan_ptr >= '0' && *num_scan_ptr <= '7')
                        num_scan_ptr++;
                } else if (*num_scan_ptr == '0' &&
                           (*(num_scan_ptr + 1) == 'b' || *(num_scan_ptr + 1) == 'B')) { // Binary
                    num_scan_ptr += 2;
            ...
            // 4. Keywords and Functions
                ...
                gboolean is_word_boundary =
                    (*word_scan_ptr == '\0' || (!g_ascii_isalnum(*word_scan_ptr) && *word_scan_ptr != '_'));
        ...
        if (token_len > 0) {
            ...
            if (!append_and_highlight(highlighted_line_gstring, start_of_plain_text, current_token_start, token_color,
                                      token_len)) {
                return FALSE; // Memory allocation failed
            }
            ptr += token_len;
            ...
            if (!append_and_highlight(highlighted_line_gstring, start_of_plain_text, current_token_start, NULL, 1))
                return FALSE;
            ptr++;
...
char *highlight_python_syntax(const char *code, gboolean show_line_numbers) {
    ...
    for (char **line_ptr = code_lines; *line_ptr != NULL; line_ptr++) {
        ...
        if (show_line_numbers) {
            ...
            g_string_append_printf(current_line_gstring, "<span foreground='#545c7e'>%*d </span>", line_number_width,
                                   current_line_num);
        }
        ...
        g_string_free(current_line_gstring,
