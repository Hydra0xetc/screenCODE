...
static gboolean append_and_highlight(
    ...
    if (!escaped_token) { g_free(token_text); return FALSE; }
...
void init_syntax_tables_go() {
    ...
    const char* standard_functions[] = {
        ...
        // fmt package
        "Errorf", "Fprint", "Fprintf", "Fprintln", "Fscan", "Fscanf", "Fscanln",
        "Print", "Printf", "Println", "Scan", "Scanf", "Scanln", "Sprint", "Sprintf",
        "Sprintln", "Sscan", "Sscanf", "Sscanln",

        // os package
        "Args", "Chdir", "Chmod", "Chown", "Chtimes", "Clearenv", "Create", "DevNull",
        "Environ", "Executable", "Exit", "Expand", "ExpandEnv", "FindProcess", "Getegid",
        "Getenv", "Geteuid", "Getgid", "Getgroups", "Getpagesize", "Getpid", "Getppid",
        "Getuid", "Getwd", "Hostname", "IsExist", "IsNotExist", "IsPathSeparator",
        "Lchown", "Link", "LookupEnv", "Mkdir", "MkdirAll", "NewFile", "NewSyscallError",
        "Open", "OpenFile", "Readlink", "Remove", "RemoveAll", "Rename", "SameFile",
        "Setenv", "StartProcess", "Symlink", "TempDir", "Truncate", "Unsetenv",

        // strings package
        "Compare", "Contains", "ContainsAny", "ContainsRune", "Count", "EqualFold",
        "Fields", "FieldsFunc", "HasPrefix", "HasSuffix", "Index", "IndexAny", "IndexByte",
        "IndexFunc", "IndexRune", "Join", "LastIndex", "LastIndexAny", "LastIndexByte",
        "LastIndexFunc", "Map", "Repeat", "Replace", "ReplaceAll", "Split", "SplitAfter",
        ...
        "TrimPrefix", "TrimRight", "TrimRightFunc", "TrimSpace",

        // strconv package
        "AppendBool", "AppendFloat", "AppendInt", "AppendQuote", "AppendQuoteRune",
        "AppendQuoteRuneToASCII", "AppendQuoteToASCII", "AppendUint", "Atoi", "CanBackquote",
        "FormatBool", "FormatFloat", "FormatInt", "FormatUint", "IsPrint", "Itoa",
        "ParseBool", "ParseFloat", "ParseInt", "ParseUint", "Quote", "QuoteRune",
        "QuoteRuneToASCII", "QuoteToASCII", "Unquote", "UnquoteChar",

        // encoding/json package
        "Marshal", "MarshalIndent", "NewDecoder", "NewEncoder", "Unmarshal", "Valid",

        // io/ioutil package (and equivalents in io, os)
        "ReadAll", "ReadFile", "WriteFile", "NopCloser", "ReadDir",
...
static gboolean highlight_tokens_on_line(GString* highlighted_line_gstring, const char* line_content, gboolean *in_multiline_comment, gboolean *next_word_is_func_name) {
    ...
    while (*ptr != '\0') {
        ...
        if (*ptr == '/' && *(ptr + 1) == '*') {
            ...
            while (*scan_ptr != '\0' && *scan_ptr != '\n') {
                if (quote_char != '`' && *scan_ptr == '\\' && *(scan_ptr + 1) != '\0') {
                    scan_ptr++;
            ...
            if (is_word_boundary) {
                if (*next_word_is_func_name) {
                    token_color = NULL; // It's a function name after 'func', don't color it.
...
char* highlight_go_syntax(const char* code, gboolean show_line_numbers) {
    ...
    if (!final_highlighted_code) return NULL;
    ...
    for (char **line_ptr = code_lines; *line_ptr != NULL; line_ptr++) {
        ...
        if (!highlight_tokens_on_line(current_line_gstring, *line_ptr, &in_multiline_comment, &next_word_is_func_name)) {
